...
internal class FirLocalVariableAssignmentAnalyzer {
    ...
    private val scopes: Stack<Pair<Fork?, MutableMap<FirProperty, MutableList<Assignment>>>> = stackOf()
    ...
    private fun isStableType(assignments: List<Assignment>?, targetType: ConeKotlinType?, session: FirSession): Boolean {
    ...
    private fun enterScope(
    ...
    ): Pair<Fork?, MutableMap<FirProperty, MutableList<Assignment>>> {
        val currentInfo = getInfoForDeclaration(symbol)
        val prohibitInThisScope = scopes.top().second.toMutableMap()
        ...
        if (!evaluatedInPlace) {
            for ((outerInfo, prohibitInOuterScope) in scopes.all()) {
                ...
                //   KT-59692
                prohibitInOuterScope.addAll(currentInfo?.assignedInside)
                // => any write to a variable outside the callable invalidates smart casts inside it
                prohibitInThisScope.addAll(outerInfo?.assignedLater)
            }
        }
        return scopes.top()
    }
    ...
    fun enterFunction(function: FirFunction) {
        if (rootFunction == null) {
            ...
            scopes.push(null to mutableMapOf())
            return
        }
        ...
        for (concurrentLambdas in postponedLambdas.all()) {
            for ((otherLambda, dataFlowOnly) in concurrentLambdas) {
                if (!dataFlowOnly && otherLambda != info) {
                    prohibitSmartCasts.addAll(otherLambda.assignedInside)
                }
            }
        }
    }
    ...
    companion object {
         ...
         */
        private class Fork(
            val assignedLater: Map<FirProperty, List<Assignment>>,
            val assignedInside: Map<FirProperty, List<Assignment>>,
        )
        ...
        private fun MutableMap<FirProperty, MutableList<Assignment>>.addAll(other: Map<FirProperty, List<Assignment>>?) {
            if (other == null) return
            for ((property, assignments) in other) {
                getOrPut(property) { mutableListOf() }.addAll(assignments)
            }
        }

        private class MiniFlow(val parents: Set<MiniFlow>) {
            val assignedLater: MutableMap<FirProperty, MutableList<Assignment>> = mutableMapOf()
        ...
        private class MiniCfgBuilder : FirVisitor<Unit, MiniCfgBuilder.MiniCfgData>() {
            ...
            private fun visitElementWithLexicalScope(element: FirElement, data: MiniCfgData): Map<FirProperty, List<Assignment>> {
                ...
                data.flow = flow
                element.acceptChildren(this, data)
                return flow.assignedLater.apply { keys.retainAll(freeVariables) }
            }
            ...
            private fun MiniCfgData.recordAssignment(reference: FirReference) {
                ...
                flow.recordAssignments(mapOf(property to listOf(assignment)))
            }

            private fun MiniFlow.recordAssignments(properties: Map<FirProperty, List<Assignment>>) {
